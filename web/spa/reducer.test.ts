...
describe("reducer", () => {
  ...
  it("SET_COUNTRY sets the id and marks the country as chosen", () => {
    const next = reducer(baseState, { type: "SET_COUNTRY", payload: { countryID: "FR" } });
  ...
  it("SET_LANGUAGE switches the UI language (used by the app surfaces)", () => {
    const next = reducer(baseState, { type: "SET_LANGUAGE", payload: { lang: "fr" } });
    expect(next.lang).toBe("fr");
  });
